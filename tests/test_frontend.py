...
class FrontendRegistrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    def test_panel_bundle_exposes_home_health_visual_tab(self) -> None:
        ...
        self.assertIn('label: "Home Care"', source)
        self.assertIn('data-tab="${tab.id}"', source)
        self.assertIn('activeTab: "right-now"', source)
        self.assertIn("renderHomeHealth()", source)
        self.assertIn("Recent care in", source)
        self.assertIn("Next care for", source)
        self.assertIn("area-next", source)
        self.assertNotIn("Helped lately", source)
        self.assertNotIn("Could help next", source)
        ...
        self.assertIn("mdi:target", source)
        self.assertIn("mdi:sparkles", source)
        self.assertIn("rejectedWhileThere = this.state.rejectedWhileThere.filter", source)
        self.assertIn("-webkit-line-clamp: 2", source)
        self.assertIn(".meta-chip.icon-chip { --hk-chip-icon-width: 34px; gap: 0; padding: 0; overflow: hidden; }", source)
        self.assertNotIn("moodLabel", source)
        self.assertIn("Home care stats", source)
        self.assertIn("Area care", source)
        self.assertIn("activeChores", source)
        self.assertIn("completedChores", source)
        self.assertIn("orderedChores", source)
        self.assertIn("renderSessionTopAction", source)
        self.assertIn("session-live-slot", source)
        self.assertIn("flash-slot", source)
        self.assertIn("dismiss-summary", source)
        self.assertNotIn("renderSessionExit", source)
        self.assertNotIn("session-exit", source)
        self.assertNotIn("renderEnding", source)
        self.assertNotIn("mdi:heart-pulse", source)
