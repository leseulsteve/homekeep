...
class TestPlanningHelpers(unittest.TestCase):
    ...
    def test_storage_counter_due_date_scales_remaining_freshness_by_food_type(self) -> None:
        today = date(2026, 1, 1)

        self.assertEqual(
            planning.storage_counter_due_date(
                "2026-01-02",
                source_location_type="fridge",
                destination_location_type="freezer",
                meal_component_family="fish",
                today=today,
            ),
            "2026-01-31",
        )
        self.assertEqual(
            planning.storage_counter_due_date(
                ...
                source_location_type="fridge",
                destination_location_type="freezer",
                meal_component_family="poultry",
                today=today,
            ),
            "2026-02-10",
        )

    def test_storage_counter_due_date_thaws_without_resetting_or_reviving(self) -> None:
        today = date(2026, 1, 1)

        self.assertEqual(
            planning.storage_counter_due_date(
                "2026-01-31",
                source_location_type="freezer",
                destination_location_type="fridge",
                meal_component_family="fish",
                today=today,
            ),
            "2026-01-02",
