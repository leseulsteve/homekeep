...
panel.hass = {
  ...
  callWS: async (message) => {
    ...
    containers: [{
      ...
      storage_started_at: "2026-06-17T12:00:00Z",
      ...
      tag_id: "demo:older-curry",
      name: "Square deli cup",
      item_label: "Older chicken curry",
      quantity: 1,
      unit: "portion",
      canonical_quantity: 1,
      canonical_unit: "portion",
...
assert.match(panel.shadowRoot.innerHTML, /Best before/);
...
assert.match(panel.shadowRoot.innerHTML, /mdi:calendar-clock/);
assert.match(panel.shadowRoot.innerHTML, /Best-before date unavailable/);
assert.match(panel.shadowRoot.innerHTML, /mdi:calendar-question/);
...
assert.match(panel.shadowRoot.innerHTML, /title="Mark eaten" aria-label="Mark eaten"/);
assert.match(panel.shadowRoot.innerHTML, /mdi:food-off-outline/);
...
assert.deepEqual(
  panel._inventoryReadyToEatSoon(panel._data.containers, panel._data.locations).map((container) => container.tag_id),
  ["demo:older-curry", "demo:frozen-tv-dinner", "demo:sauce", "demo:thawed-stew"],
);
...
assert.deepEqual(panel._tvDinnerTransferMeals(panel._tvDinnerPlan, panel._availableTvDinnerContainers(staleTvDinnerData), staleTvDinnerData), [{
  ...
  components: {
    veggie: { tag_id: "demo:braised-greens", label: "Braised greens", quantity: 1, role: "", family: "leafy_green", detail: "greens", best_before_date: "" },
    starch: { tag_id: "demo:sweet-potatoes", label: "Sweet potatoes", quantity: 1, role: "", family: "potato", detail: "sweet_potato", best_before_date: "" },
    protein: { tag_id: "demo:salmon", label: "Salmon portions", quantity: 1, role: "", family: "fish", detail: "salmon", best_before_date: "" },
  },
}]);
...
assert.deepEqual(serviceCalls.at(-1), {
  ...
  data: {
    meals: [{
      ...
      components: {
        veggie: { tag_id: "demo:broccoli", label: "Roasted broccoli", quantity: 1, role: "", family: "cruciferous", detail: "broccoli", best_before_date: "" },
        starch: { tag_id: "demo:meal-rice", label: "Cooked basmati rice", quantity: 1, role: "", family: "rice", detail: "basmati_rice", best_before_date: "" },
        protein: { tag_id: "demo:curry", label: "Chicken curry", quantity: 1, role: "", family: "poultry", detail: "chicken", best_before_date: "" },
      },
    }],
  },
});
...
assert.match(panel.shadowRoot.innerHTML, /<span>Contents<\/span><strong>3 meal · 2 empty<\/strong>/);
...
assert.match(panel.shadowRoot.innerHTML, /Top shelf · 5 containers/);
...
panel._containerContentKind = "recipe";
panel._render();
assert.match(panel.shadowRoot.innerHTML, /<option value="recipe" selected>Recipe batch<\/option>/);
assert.match(panel.shadowRoot.innerHTML, /<label>Best before<input name="best_before_date" type="date" \/><\/label>/);
assert.doesNotMatch(panel.shadowRoot.innerHTML, /<label>Purchased<input name="purchased_date"/);
panel._containerContentKind = "ingredient";
