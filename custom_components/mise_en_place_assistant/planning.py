...
def storage_counter_due_date(
    best_before_date: Any,
    *,
    source_location_type: Any,
    destination_location_type: Any,
    content_kind: Any = "",
    storage_behavior: Any = "",
    meal_component_role: Any = "",
    meal_component_family: Any = "",
    ...
    """Convert remaining freshness between storage types without resetting it."""
    parsed = parse_inventory_date(best_before_date)
    if not parsed:
        return None
    today = today or date.today()
    remaining_days = (parsed - today).days
    if remaining_days <= 0:
        return parsed.isoformat()
    source_days = storage_due_days(
    ...
    destination_days = storage_due_days(
    ...
    if not source_days or not destination_days:
        return parsed.isoformat()
    adjusted_remaining = max(0, math.ceil(remaining_days * destination_days / source_days))
    return date.fromordinal(today.toordinal() + adjusted_remaining).isoformat()
