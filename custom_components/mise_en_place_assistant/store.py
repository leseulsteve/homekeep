...
from .planning import freshness_rank_key, meal_candidate_rank, storage_counter_due_date, storage_due_date
...
class MiseEnPlaceAssistantInventory:
    ...
    def _ensure_schema(self) -> bool:
        ...
        for tag_id, container in self.containers.items():
            ...
            if "updated_at" not in container:
                ...
                changed = True
            if "storage_started_at" not in container:
                container["storage_started_at"] = container.get("updated_at") or container.get("created_at") or _utc_now()
                changed = True
        ...
        *,
        ...
        for key, value in (
            ...
            if value not in (None, ""):
                ...
                key in old
                and float(old.get("canonical_quantity", old.get("quantity", 0)) or 0) > 0
                and old.get("product_id") == candidate.get("product_id")
                and old.get("content_kind") == candidate.get("content_kind")
            ):
                candidate[key] = old[key]
        if float(candidate.get("canonical_quantity", candidate.get("quantity", 0)) or 0) <= 0:
            self._clear_freshness_fields(candidate)
    ...
    async def async_create_recipe_container(
        ...
        sublocation: str | None = None, best_before_date: str | None = None,
    ) -> None:
    ...
    async def _async_update_container(
        ...
        *,
        ...
        if location is not None or location_id is not None:
            ...
            if candidate.get("location_id") != before.get("location_id"):
                candidate["storage_started_at"] = now
        ...
        if before.get("product_id") != candidate.get("product_id") or before.get("content_kind") != candidate.get("content_kind"):
            self._clear_freshness_fields(candidate)
        for key, value in (
            ("best_before_date", best_before_date),
            ("purchased_date", purchased_date),
        ...
        candidate.setdefault("storage_started_at", before.get("storage_started_at") or before.get("updated_at") or before.get("created_at") or now)
    ...
    async def async_transfer_tv_dinners(self, meals: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for index, meal in enumerate(meals, start=1):
            ...
            for role in ("veggie", "starch", "protein"):
                ...
                source_location = self.location_for_id(source.get("location_id")) or {}
                ...
                normalized_components[role] = {
                    ...
                    "location_type": source_location.get("location_type") or "",
                    "best_before_date": source.get("best_before_date") or "",
                }
        ...
        for meal, target in zip(normalized_meals, targets, strict=True):
            ...
            best_before_date = self._tv_dinner_best_before_date(meal["components"], fridge)
            await self._async_create_container(
            ...
            created.append(
                {
                    ...
                    "container_type": "tv dinner",
                    "label": meal_label,
                    "best_before_date": best_before_date or "",
    ...
    @staticmethod
    @staticmethod
    def _tv_dinner_best_before_date(components: dict[str, dict[str, Any]], location: dict[str, Any]) -> str | None:
        """Return the least-fresh transformed component date in the assembled TV dinner."""
        destination_type = location.get("location_type") or "fridge"
        best_before_dates = []
        for component in (components or {}).values():
            if not component.get("best_before_date"):
                continue
            best_before_dates.append(
                storage_counter_due_date(
                    component.get("best_before_date"),
                    source_location_type=component.get("location_type") or destination_type,
                    destination_location_type=destination_type,
                    content_kind="meal",
                    meal_role="prepared_component",
                    meal_component_role=component.get("role") or "",
                    meal_component_family=component.get("family") or "",
                )
                or component.get("best_before_date")
            )
        if not best_before_dates:
            return None
        return min(best_before_dates, key=freshness_rank_key)
    ...
    def _apply_storage_due_date(
        ...
        *,
        ...
        """Adjust best-before date when an item moves between typed storage."""
        ...
        same_item = (
            (before or {}).get("product_id") == candidate.get("product_id")
            and (before or {}).get("content_kind") == candidate.get("content_kind")
        )
        if source.get("location_type") == destination_type and same_item:
            return
        ...
        due_date = None
        if same_item and (before or {}).get("best_before_date"):
            due_date = storage_counter_due_date(
                (before or {}).get("best_before_date"),
    ...
    @staticmethod
    def _clear_freshness_fields(container: dict[str, Any]) -> None:
        """Remove freshness fields once a container no longer holds food."""
        for key in (
