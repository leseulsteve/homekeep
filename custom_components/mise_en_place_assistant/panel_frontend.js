class MiseEnPlaceAssistantPanel extends HTMLElement {
    ...
    return `
      <form class="card form" id="create-form">
        ...
        <div class="form-grid">
          ...
          ${["ingredient", "recipe", "meal"].includes(this._containerContentKind) ? `<label>Best before<input name="best_before_date" type="date" /></label>` : ""}
          ${this._containerContentKind === "ingredient" ? `<label>Purchased<input name="purchased_date" type="date" /></label><label>Opened<input name="opened_date" type="date" /></label>` : ""}
        </div>
  ...
  _tvDinnerTransferMeals(plan = {}, availableContainers = [], data = {}) {
    ...
    return (plan?.meals || []).map((meal, index) => ({ meal, index })).filter(({ meal }) => {
      ...
      return {
        ...
        components: {
          veggie: this._tvDinnerTransferComponent(components.veggie, currentContainers),
          starch: this._tvDinnerTransferComponent(components.starch, currentContainers),
          protein: this._tvDinnerTransferComponent(components.protein, currentContainers),
        },
      };
    });
  }

  _tvDinnerTransferComponent(component = {}, currentContainers = new Map()) {
    ...
    return {
      ...
      best_before_date: container.best_before_date || component.best_before_date || "",
    };
  }
  ...
  _inventoryReadyToEatSoon(containers = [], locations = []) {
    ...
    return containers
      ...
      .sort((left, right) => {
        const leftKey = this._inventoryReadyToEatFreshnessKey(left, locationTypes.get(left.location_id) || "");
        const rightKey = this._inventoryReadyToEatFreshnessKey(right, locationTypes.get(right.location_id) || "");
  ...
  _inventoryReadyToEatFreshnessKey(container = {}, locationType = "") {
    ...
    const freshnessDate = bestBefore ? String(container.best_before_date).slice(0, 10) : "";
    return {
      group: freshnessDate ? 0 : 1,
      date: freshnessDate || status.sortDate || "9999-12-31",
      rank: status.rank,
      label: String(container.item_label || container.name || container.tag_id || ""),
    };
  }
  ...
  _inventoryReadyToEatRow(container, locations = []) {
    ...
    return `
      <tr>
        ...
        <td>${this._inventoryReadyBestBeforeBadge(container)}</td>
        <td><button type="button" class="icon-button" data-mark-container-eaten="${this._safe(container.tag_id)}" data-container-name="${this._safe(label)}" title="Mark eaten" aria-label="Mark eaten"><ha-icon icon="mdi:food-off-outline"></ha-icon></button></td>
      </tr>
    `;
  }
  ...
  _inventoryReadyBestBeforeBadge(container = {}) {
    const bestBefore = this._parseInventoryDate(container.best_before_date);
    if (!bestBefore) {
      return `<span class="ready-age muted" title="Best-before date unavailable"><ha-icon icon="mdi:calendar-question"></ha-icon><strong>-</strong><span>date</span></span>`;
    }
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const days = Math.round((bestBefore.getTime() - today.getTime()) / 86400000);
    const klass = days < 0 || days <= 1 ? "critical" : days <= 7 ? "warn" : "";
    const count = Math.abs(days);
    const unit = count === 1 ? "day" : "days";
    const label = days < 0 ? "over" : days === 0 ? "today" : unit;
    const title = `Best before ${this._relativeDateLabel(container.best_before_date)}`;
    return `<span class="ready-age ${this._safe(klass)}" title="${this._safe(title)}"><ha-icon icon="mdi:calendar-clock"></ha-icon><strong>${this._safe(count)}</strong><span>${this._safe(label)}</span></span>`;
  }
  ...
  _inventoryReadyStorageAge(container = {}, locationType = "") {
    const storedSince = this._parseInventoryDateTime(container.storage_started_at || container.updated_at || container.created_at);
